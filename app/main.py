...
def require_local_operation():
    if APP_MODE == "cloud":
        raise HTTPException(
            status_code=403,
            detail="This operation is available only on the local server.",
        )
...
async def set_scale_weight(
    ...
    if APP_MODE == "cloud":
        raise HTTPException(
            status_code=403,
            detail="Scale writes are disabled on cloud.",
        )
